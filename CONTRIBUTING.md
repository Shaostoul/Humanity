...
```
Humanity/
├── *.html              ← 18 standalone pages (one per nav entry)
├── shared/
...
│       └── client/             ← Chat page + all chat JS/CSS
│           ├── index.html
...
The chat client lives in `crates/humanity-relay/client/`. It's split into modules loaded in order:
...
The relay server is a Cargo workspace crate at `crates/humanity-relay/`.
