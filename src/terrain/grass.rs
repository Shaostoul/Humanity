...
//!     and the live-emergence pair. They MOVED OUT in 2026-09-19 under the
//!     ratchet, to the `grass_fields` child module, and are re-exported here
//!     so every `grass::` and `planet_chunks::` path still resolves;
...
/// The pure FIELDS the layer is built on (2026-09-19): distance to density,
/// and position to clumping, height and dryness. Another `#[path]` CHILD for
/// the same reason `grass_mesh` is one - it reads the `GRASS_*` constants
/// through a single `use super::*`, and this re-export keeps every existing
/// `grass::grass_density_at` / `planet_chunks::grass_clump_gain` path working.
#[path = "grass_fields.rs"]
mod grass_fields;
pub use grass_fields::*;

/// The SOURCE TEXT of the grass COVERAGE path, for the two gates that prove a
/// thing by reading the code rather than by running it: the quality-slider
/// scan in `near_grass_density_matches_a_real_sward` (which cannot measure a
/// re-introduced slider read, because the test process runs at the atomic's
/// 1.0 default) and the Settings page's `engine_source`.
///
/// It is a LIST because the coverage path is now two files, and the v0.1320
/// lesson is that a scan naming one file reports a wiring break the moment
/// anything moves. ADD ANY FILE YOU EXTRACT FROM THE COVERAGE PATH HERE, in
/// the same commit.
///
/// `grass_mesh.rs` is deliberately NOT on this list: the quality slider is
/// SUPPOSED to be read there (`grass_detail_for` is the one place it may be
/// interpreted), so including it would make the scan flag the correct code.
pub const COVERAGE_PATH_FILES: &[&str] = &["src/terrain/grass.rs", "src/terrain/grass_fields.rs"];

/// Each coverage-path file paired with its own text, so a scanner can report
/// `file:line` rather than a line number into a concatenation (which is what
/// the first draft of this did, and it pointed at a line grass.rs does not
/// have).
pub fn coverage_path_files() -> Vec<(&'static str, String)> {
    let root = std::path::Path::new(env!("CARGO_MANIFEST_DIR"));
    COVERAGE_PATH_FILES
        .iter()
        .map(|rel| {
            let txt = std::fs::read_to_string(root.join(rel))
...
/// The same thing as one blob, for scanners that only ask "does this text
/// contain X".
pub fn coverage_path_source() -> String {
    coverage_path_files()
        .into_iter()
        .map(|(_, t)| t)
        .collect::<Vec<_>>()
...
mod tests {
    ...
    fn near_grass_density_matches_a_real_sward() {
        ...
        // EVERY file of the coverage path, not just this one: the ramp and the
        // position fields moved to grass_fields.rs in 2026-09-19, and a scan
        // that still named one file would have gone quietly blind to half of
        // what it guards. Scanned file by file so an offender is reported as
        // `file:line` - a line number into the concatenation pointed at a line
        // grass.rs does not have. See `coverage_path_files`.
        //
        ...
        let offenders: Vec<String> = coverage_path_files()
            .iter()
            .flat_map(|(rel, txt)| {
                txt.lines()
                    .enumerate()
        ...
        assert!(
            offenders.is_empty(),
            "the quality slider is read in the coverage path at {offenders:?} \
