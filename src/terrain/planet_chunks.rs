...
/// The WATER SHELL's own patch meshes and the three `WATER_*` constants that
/// size them (2026-09-19 extraction - see the ratchet notes). Another
/// `#[path]` child, so `use super::*` gives it `RadialBand`, `PatchId` and
/// `PatchMesh`, and this re-export keeps every `chunks::water_band` /
/// `chunks::build_water_patch_mesh_at` call in `engine/frame_water.rs`
/// resolving unchanged.
#[path = "water_patches.rs"]
mod water_patches;
pub use water_patches::*;
