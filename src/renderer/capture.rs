...
//!
//! WHAT IS NO LONGER HERE (2026-09-19, the ratchet again): the off-screen
//! VIEW DEPTH swap - `set_depth_target_size`, `begin_view_depth`,
//! `end_view_depth` and the `ViewDepth` enum - moved to
//! `renderer/view_depth.rs`. It was never really capture: it is what an
//! off-screen render needs before any pixels exist, and its other caller (the
//! in-world camera screens) captures nothing at all.
...
impl Renderer {
    ...
    ///
    /// (This doc comment had drifted away from its function: an earlier
    /// extraction left it stranded above `dump_cloud_map_png`, where it read
    /// as the first half of that function's description. Put back.)
