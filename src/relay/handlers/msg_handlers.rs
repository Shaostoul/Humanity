...
// ── Stream handlers ──
//
// Livestreaming moved to its own file in 2026-09-19 under the file-size
// ratchet: eleven handlers, a WebRTC signalling triangle and their own test
// module, all talking only to each other and to `state.live_streams`. A
// `#[path]` CHILD, so `use super::*` gives it everything this file has and
// the re-export keeps every `handle_stream_*` call in relay.rs resolving.
#[path = "stream.rs"]
mod stream;
pub use stream::*;
