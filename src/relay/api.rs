...
use crate::relay::relay::{RelayMessage, RelayState, Peer, PeerInfo, SearchResultData};
    ...
    match state.db.create_task_in_project(&req.title, &req.description, status, priority, req.assignee.as_deref(), "bot_api", &req.labels, proj) {
        Ok(id) => {
            ...
            if let Ok(Some(task)) = state.db.get_task(id) {
                let td = crate::relay::relay::TaskData {
    ...
    match state.db.create_project(&id, &req.name, &req.description, "bot_api", vis, &req.color, &req.icon) {
        Ok(()) => {
            ...
            if let Ok(Some(rec)) = state.db.get_project_by_id(&id) {
                let pd = crate::relay::relay::ProjectData {
...
// ── Marketplace API ──
//
// Listings, their images, their reviews, the seller rating those reviews
// summarise, and the order book moved to their own file in 2026-09-19 under
// the file-size ratchet: three banners that were always one feature. A
// `#[path]` CHILD, so `use super::*` gives it the axum extractors and the
// auth helpers this file has, and the re-export keeps every `api::` name the
// router calls resolving.
#[path = "api_market.rs"]
mod api_market;
pub use api_market::*;
...
pub async fn list_files(
    ...
    match crate::relay::storage::files::list_directory(dir_path) {
...
pub async fn read_file(
    ...
    match crate::relay::storage::files::read_file(&params.path) {
        Ok(content) => {
            let abs_path = crate::relay::storage::files::validate_path(&params.path);
...
pub async fn write_file(
    ...
    match crate::relay::storage::files::write_file(&req.path, &req.content) {
