...
impl Storage {
    ...
    // (Listing buyer-seller message storage removed 2026-08-23:
    // marketplace contact rides sealed-sender E2EE DMs now, so the relay
    // stores no marketplace correspondence. The listing_messages table is
    // dropped by migration in storage/mod.rs.)
}
