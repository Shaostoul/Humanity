...
pub struct GuiState {
    ...
    /// In-app Library: nested categories of reference docs (`data/library/`).
    pub library: Vec<LibraryCategory>,
...
/// One reference document in the in-app Library (title + raw markdown body).
pub struct LibraryDoc {
    pub title: String,
    pub body: String,
}

/// A named, nested category of Library documents (`data/library/index.json`).
pub struct LibraryCategory {
    pub name: String,
    pub docs: Vec<LibraryDoc>,
}

/// Load the in-app Library (`data/library/index.json` plus the markdown files it
/// references) into nested categories. Empty vec on any error, so the page falls
/// back to a "no documents" note. Generated by `scripts/build-library.js`.
#[cfg(feature = "native")]
pub fn load_library(data_dir: &std::path::Path) -> Vec<LibraryCategory> {
    #[derive(serde::Deserialize)]
    struct DocEntry {
        title: String,
        file: String,
    ...
    let manifest = match read_data_json::<Manifest>(data_dir, "library/index.json") {
        Some(m) => m,
        None => return Vec::new(),
    };
            ...
            docs: c
                ...
                .filter_map(|d| {
                    std::fs::read_to_string(dir.join(&d.file))
                        .ok()
                        .map(|body| LibraryDoc { title: d.title, body })
                })
                .collect(),
        })
        .collect()
