...
fn draw_body(ui: &mut egui::Ui, theme: &Theme, state: &mut GuiState) -> bool {
    ...
    // Performance plan increment 4, the far rung: beyond the footprint band
    // the march reads a planet-fixed cloud PROFILE (fraction, mean density,
    // column per lattice cell and height bin) instead of point-sampling the
    // constructed bodies - the orbit speckles. The checkbox is the A/B (off
    // = knob 0, the exact old path); the level picker beside it selects the
    // dev modes the gates use: Auto (blended by footprint), Hard (the
    // prove-red hard switch), Ref (the slow reference bake), L0..L5 (one
    // level forced on every sample). Channels 10/11/12 below paint the
    // share, the level and the fraction.
    ...
    if state.cloud_dev_profile_knob != 0 {
        // (knob, label): the knob VALUE is what the shader's CLOUD_FR_KNOB_*
        // codes read (1 on, 8 hard, 9 ref, 2..7 = level 0..5 forced).
        let modes: [(i32, &str); 9] = [
            (1, "Auto"),
            (8, "Hard"),
            (9, "Ref"),
    ...
    // 10/11/12 (increment 4, the far rung): the profile SHARE (w_pf, white
    // = drawn from the planet-fixed profile), the profile LEVEL (level / 6,
    // the window staircase about the nadir; 6 = the global) and the profile
    // FRACTION (f; against the A17 synthetic atlas the planet-fixed i / 512
    // sawtooth).
