...
impl Storage {
    ...
    pub fn get_dm_conversations(&self, my_key: &str) -> Result<Vec<DmConversation>, rusqlite::Error> {
        ...
        // Use plain `?` (positional) rather than `?N` (numbered) so that three IN
        // clauses each get their own distinct param values via chain(chain(chain)) below.
        // With ?N params SQLite deduplicates by index, causing a param-count mismatch.
