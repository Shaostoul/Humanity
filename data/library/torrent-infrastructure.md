...
> releases, v0.1296.0 through v0.1304.0, with 140 asset entries and ZERO
> `magnet` fields, and the `.torrent` sibling of a release archive
> returns 404 while the archive itself returns 200. So the two
> bandwidth-friendly fetch methods described below do not currently
> exist, and HTTPS is the only working path. Either the seeding half of
> `regen-releases-manifest` needs restoring on the VPS, or this document
> and `distribution-mirrors.md` step 4 should stop claiming the layer is
> live. Do not point a user at a magnet from here until the manifest
> carries one again.

The design below is what the VPS runs when the seeder is active: an
always-on BitTorrent seeder for every release bundle, with magnet URIs
published in the manifest at
...
> **BROKEN AS OF 2026-09-14: the Forgejo host fails TLS hostname
> verification.** The certificate served for `git.united-humanity.us` has
> `CN=united-humanity.us` and a SAN list of `united-humanity.us` and
> `chat.united-humanity.us` only. The git subdomain is not in it, so
> every client that validates TLS refuses the connection: curl returns
> status 000 and connects only with `-k`. Each shipped data manifest
> carries hundreds of per-file URLs pointing at that host, so the
> delta-sync path is dead for any correct client. The fix is on the
> server, not here: reissue the certificate with the subdomain included,
> for example
> `sudo certbot --nginx -d united-humanity.us -d chat.united-humanity.us -d git.united-humanity.us`
> then reload nginx. If the Forgejo mirror is no longer meant to be the
> per-file CDN, change the base URL in `scripts/gen-data-manifest.js`
> instead and rewrite this section.
