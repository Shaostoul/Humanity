...
- **HIGH-2 — no proof-of-possession at `identify` (identity is spoofable). ❌ OPEN — gating item before ANY real-user / public launch.** The relay binds a socket to a self-asserted `public_key` with no challenge-response, so anyone can claim another user's Dilithium identity (seize names, receive their relayed DMs — though they CANNOT decrypt them: the DM is Kyber-sealed to the victim's seed-derived key, so **DM confidentiality holds**). This is **pre-existing** (the Ed25519 era was identical) and is the runbook's already-isolated **Inc3b**: on `identify`, relay issues a nonce → client returns a Dilithium signature over it → relay verifies before binding the socket. Touches relay+web+native identify. Do it as its own focused increment; it is the top pre-launch security gate. It does NOT make the Inc6 wipe more dangerous (the wipe just resets data; HIGH-2 is an ongoing protocol gap equally present before/after) and does NOT block the operator's own attended DM verification — but real users must not onboard until Inc3b ships.
- **MED-1** (chat "absent pq_signature → allow") auto-resolves once native chat signs (the deferred Inc4 follow) + HIGH-2 lands: then flip to "absent → reject".
...
- **HIGH-2 / Inc3b** (above) — the top security gate before public launch.
- **Inc5c-tail**: standalone web pages still Ed25519-sign relay auth (broken since Dilithium became the identity — NOT a new regression; relay is at the correct end-state): `web/pages/settings-app.js` (vault sync), `admin-app.js` (`admin_stats`), `market-app.js` (listing reviews). Each needs pq.js + Dilithium-from-seed in its own page identity context. Native does NOT client-call these (local config).
- Native chat Dilithium-signing at send sites (Inc4 deferred follow; helper `net::identity::pq_sign_chat` exists).
- Inc6 (attended, operator): deploy → `just pq-wipe yes` (re-seeds #announcements) → operator re-onboards web+native from seed and verifies a cross-client DM round-trips. (No `just security-review` recipe exists — review is done via an independent agent; this section IS its outcome.)
