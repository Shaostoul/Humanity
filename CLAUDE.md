...
- **Windows PowerShell `git commit -m` mangles messages containing embedded double quotes** (PS 5.1 native-arg re-quoting splits the message; git treats the tail as pathspecs — this is how the stray `v0.415.0` tag landed on the wrong commit, 2026-06-11). For any multi-line message, write it to a temp file and use `git commit -F <file>` / `gh release create --notes-file <file>`. The stray `v0.415.0` tag points at the v0.414.0 commit and has NO release — per the never-delete/re-tag SOP we incremented past it; don't "fix" it and don't create a release for it.
