...
- **Every fragment of every pipeline that uses `fs_main` pays the megashader's per-invocation private frame (H1, proven 2026-09-18, `docs/design/frame-cost-arc.md`).** The cloud march's 2.9 KB `var<private>` array was charged to every terrain and interior pixel with clouds OFF: 44.5 ms for six moon patches. Since P2 (same day) the ten megashader PSOs compile PER MATERIAL CLASS through WGSL `override` constants (`assets/shaders/pbr/05-overrides.wgsl`; `pipeline.rs` `ShaderClass`, `shader_class(material_type)`, `PSO_DEAD_BRANCHES` rows per class): General has all three shell branches off (the classic and terrain PSOs), Shell keeps atmosphere and ocean (the atmosphere and water shells), Cloud keeps only the cloud march (the type-15 deck). A draw loop that can carry a shell MUST pick its PSO with `Pipeline::transparent_for` / `overlay_for` on the object's class (the opaque loops debug-assert General); a shell object on a General PSO renders as the default panel grid with no error in release. The limb went 25.0 to 0.37 ms and the console room 82.9 to 16.8 ms on the switch alone. Rules: a new heavyweight branch in `fs_main` gets its own `override` switch and is turned off in every pipeline that cannot draw it; the switch is the FIRST operand of the guard (`if (HAS_X && type_test)`), the convention the tests pin; naga IGNORES an unknown override key and returns a module with no overrides unchanged, so a rename on one side, or a stale assets mirror without the declarations, silently re-enables the branch (the `permutation_tests` in `pipeline.rs` and the switch check in `shader_loader::validate_wgsl` guard both; keep them). Measure with the rig, not by reading: the same-boot A/B through the shader hot reload is the tool.
