...
> 13. **After pushing a Rust release**: run `just deliver` (build + archive + refresh the taskbar exe), or the operator cannot see the work. `just brief` now prints a DELIVERY row that answers this directly, and `just check-delivery` answers it on demand; the VERSION row above it compares the repo to GitHub, which is NOT the same question and read "in sync" through five undelivered releases. Detail follows. CI does not build Windows, and **`just launch` boots the newest `v*_HumanityOS.exe` ARCHIVE in the repo root**, which only `just build-game` or `node scripts/archive-build.js` creates. A plain `cargo build --features native --release` updates `target/release` and NOTHING else, so an agent can ship five tagged releases while the operator keeps launching the build from before them (this happened 2026-09-21: he was on v0.1326 at v0.1331). Use `node scripts/archive-build.js` on its own when the version is already bumped and tagged, since `just build-game` bumps again and would strand local ahead of GitHub.
...
**Web MIRRORS native; native is the product (operator, 2026-07-05).** The native egui app is the source of truth for every UI pattern; the website reflects it in HTML/CSS ("just the GUI of the app is meant to inspire the website"). Serve what we can via web, but design native-first, then mirror. Don't build web-only pages that diverge from the app into a second thing to maintain (the "two partial gardening games" anti-pattern). One web-browsing surface was wanted long-term: OUR OWN lightweight browser (NOT Chromium/CEF) so real sites render on in-game monitors, seeded by web.html + the native Browser page. **AMENDED 2026-09-18:** that still holds for READING the web, which is what `src/web_reader` does. It does NOT hold for WATCHING video platforms, because their players are JavaScript applications our reader cannot run, and the operator decided he wants them on the in-world monitors: "watching YouTube, Twitch and Rumble on the in-world monitors is what we want." So an embedded Chromium is now the agreed route FOR THAT CASE ONLY, as a separate opt-in host process whose runtime is never bundled, with the platforms own players and their advertising intact. Measured 2026-09-18 (`docs/design/embedded-browser.md`): 0.064 ms GPU and under 0.63 ms CPU per 720p screen, the page served from localhost so our website is not in the path, YouTube and Rumble playing, and Twitch accepted but unable to decode its H.264 video because the freely distributable engine carries no such decoder, which is the same patent line that keeps those decoders out of our own player. Full strategic direction lives in the PRIORITIES.md "Active focus" block.
...
- **LOW-1: ✅ CLOSED.** The server-side plaintext `/dm` command was removed in v0.279.0, and the sealed-sender cutover (2026-08-23) removed the `RelayMessage::Dm` plaintext-capable variant entirely; the v2 protocol has no field that can carry an unencrypted DM.
...
> **Removed (privacy hardening, do NOT re-add):** `direct_messages` (the DM
> social graph, 2026-08-23), `listing_messages` (plaintext buyer-seller threads,
> broadcast to everyone, 2026-08-23), `groups`/`group_members`/`group_messages`
> (plaintext legacy groups, 2026-08-23), and `follows` (the last server-side
> social graph, 2026-08-24). Marketplace contact rides sealed-sender DMs; groups
> are the E2EE P2P signed-object system; following/friendship are client-side
> (sealed control DMs + client-held Dilithium friendship certificates verified
> statelessly). If a feature seems to need a server-side social edge, the
> certificate pattern (client-held, statelessly verified) is the house answer.

## Known gotchas

- **Every fragment of every pipeline that uses `fs_main` pays the megashader's per-invocation private frame (H1, proven 2026-09-18, `docs/design/frame-cost-arc.md`).** The cloud march's 2.9 KB `var<private>` array was charged to every terrain and interior pixel with clouds OFF: 44.5 ms for six moon patches. Since P2 and P3 (same day) there is NO `fs_main`: six class entries (`fs_surface`, `fs_terrain`, `fs_vegetation`, `fs_water`, `fs_shell`, `fs_cloud` in `90-fragment-main.wgsl`) share `frag_prologue` + `frag_tail` (`80-fragment-shared.wgsl`), and the thirteen megashader PSOs each compile ONE entry plus a dead-switch set (`pipeline.rs` `ShaderClass`, `shader_class(material_type)` pinned to the WGSL bands by a test, `PSO_REGISTRY` rows, `MegashaderPsos`). Every draw loop picks its PSO by the object's class (`Pipeline::opaque_for` / `transparent_for` / `overlay_for`; `draw_opaque_objects` walks an opaque list once per class present); a material on an entry that lacks its branch renders as the default panel grid with no error in release. The celestial transparent list is grouped by the planet LAYER BAND (`renderer/celestial_order.rs`), never by class: a class key put the type-13 fallback dome under the sea (P2 review). Numbers: limb 25.0 to 0.37 ms and the console room 82.9 to 16.8 on P2, then 17.1 to 14.1 on P3 (register pressure). Rules: a new heavyweight branch gets its own `override` switch, lives in exactly one class entry, and is turned off in every pipeline that cannot draw it; the switch is the FIRST operand of the guard (`if (HAS_X && type_test)`), the convention the tests pin; naga IGNORES an unknown override key and returns a module with no overrides unchanged, so a rename on one side, or a stale assets mirror without the declarations, silently re-enables the branch (the `permutation_tests` in `pipeline.rs` and the switch check in `shader_loader::validate_wgsl` guard both; keep them). Measure with the rig, not by reading: the same-boot A/B through the shader hot reload is the tool.
- **Bind-group-layout changes must update EVERY create_bind_group site, and renderer verifies must ENTER THE WORLD (incident v0.1029-v0.1038, 2026-07-28).** Adding binding 15 to the texture BGL updated two of three creation sites; the missed one (the per-material Material Albedo Bind Group, built lazily when a TEXTURED material loads) made every WORLD ENTRY panic with a wgpu validation error - while menu-only boot-verifies stayed green for TEN releases (the operator hit it: "insta crashes when I press esc"). Rules: (1) after touching a bind group layout, grep that layout name for ALL create_bind_group sites and count entries at each; (2) the boot-verify bar for renderer changes now includes WORLD ENTRY - use the probe rig (node scripts/probe-sweep.js --only blue-marble-12000km --exe target/release/HumanityOS.exe): it runs a portable sandbox (the in-place exe REFUSES autopilot when a real identity exists), enters via autopilot, and reports panics=0.
- **Renderer/device changes MUST be verified by BOOTING the release exe (incident v0.782-784, 2026-07-10).** `cargo test` + naga shader validation do NOT catch wgpu DEVICE-LIMIT rejections: the v0.782 lights storage buffer passed every test, but the device was requested with `downlevel_webgl2_defaults` (fragment storage buffers = 0), so `create_bind_group_layout` failed validation at startup and the app died before the first frame — THREE releases (v0.782.0-v0.784.1) shipped unbootable before the operator hit it ("a flicker but the game never comes up"). Limits are now `wgpu::Limits::default()` (`renderer/mod.rs` `request_device`). Verify bar for ANY renderer/pipeline/bind-group/shader change: `cargo build --features native --release`, launch `target/release/HumanityOS.exe` **with `HUMANITY_NO_FOCUS=1` in its env** (otherwise the boot-verify window steals foreground focus and kills the operator's raw-input mouse-look mid-game - the "mouse stopped aiming" incident, root-caused 2026-07-27), wait ~10 s, grep `%APPDATA%/HumanityOS/logs/run.log` for `PANIC` (expect 0), then `taskkill //PID <pid>`. When the operator reports a boot failure, read `run.log`/`crash.log` FIRST — the panic hook always writes the cause there.
- **f32 at planet scale is a defect class: never pass planet positions OR unit directions through f32 on the way to a terrain/data sampler.** An f32 UNIT vector quantizes ground position at ~0.4-0.8 m; an f32 global-grid coordinate (lon-derived, ~72k cells) quantizes at ~3.6 m - both drew real artifacts (v0.1010 terrain ripples at the operator bm-7 bookmark; v0.1012 clamp micro-stepping). Keep dirs as DVec3 end-to-end and downcast ONLY at a coarse-data boundary (base heightmap cells are 460+ m, so .as_vec3() is fine THERE and nowhere earlier). GPU/WGSL is f32 by hardware - the discipline there is small magnitudes near the camera (floating origin, anchor-relative patch verts, drawn-shell units). Audit sweep 2026-07-27 (operator request) cleared: mesh vertex path, camera/floating origin, anchors/spin, ballistics, cloud shader units. **The water arc (v0.1017-v0.1018, same day) then proved the GPU edition of the class:** any shader phase/UV computed from a planet-radius-magnitude f32 dot (`dot(dir * r, d)`) carries ~0.5 m quantization that SHIFTS with camera motion - on short wavelengths that is camera-coupled vertex jitter. The established fix pattern: evaluate in the camera-anchored 64 m-modulus domain (`anchw + dvw`, the wave-texture anchor) with wavelengths that DIVIDE 64 and axis-aligned directions, so anchor re-snaps shift phase by exact integers and the CPU f64 twin matches mod 1 exactly (ocean chop trains W4/W5/W6 = the worked example; the lockstep test guards it). Also caught: a CALLER narrowing to f32 before an f64-upgraded function (the buoyancy float) - upgrading a function's internals is worthless if the call site already quantized; check the whole chain.
