...
- **Inc5c-tail**: standalone web pages still Ed25519-sign relay auth (broken since Dilithium became the identity — NOT a new regression; relay is at the correct end-state): `web/pages/settings-app.js` (vault sync), `admin-app.js` (`admin_stats`), `market-app.js` (listing reviews). Each needs pq.js + Dilithium-from-seed in its own page identity context, which means re-prompting the user for the vault passphrase in those page contexts (they don't share state with the chat client). Discuss UX before implementing. Native does NOT client-call these (local config).
- Inc6 (attended, operator): deploy → `just pq-wipe yes` (re-seeds #announcements) → operator re-onboards web+native from seed and verifies a cross-client DM round-trips. (No `just security-review` recipe exists — review is done via an independent agent; this section IS its outcome.) HIGH-2 + MED-1 + federation gossip + PBKDF2 bump are all SHIPPED in code; Inc6 is the activation.
