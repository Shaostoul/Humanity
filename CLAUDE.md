...
- **Inc5c-tail**: the standalone web pages still sign these endpoints with Ed25519, so they're broken until re-PQ'd (each needs pq.js + Dilithium-from-seed plumbing in its own page identity context): `web/pages/settings-app.js` (vault sync `vault_sync\nts`), `web/pages/admin-app.js` (`getSignedAuth('admin_stats')`), `web/pages/market-app.js` (listing review create/delete). Native does NOT client-call these (local config) — no native work. NOTE: these were ALREADY broken the moment Dilithium became the identity (Ed25519 sig vs Dilithium `key`); the relay swap is the correct end-state, not a new regression.
- Inc6 (attended): `just security-review` on the full crypto diff → deploy → `just pq-wipe yes` (re-seeds #announcements) → operator live web↔native DM verify.
