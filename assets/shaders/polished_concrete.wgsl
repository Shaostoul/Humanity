// polished_concrete.wgsl — Procedural polished concrete with aggregate and expansion joints.
// Light gray base with visible aggregate chunks, fine polishing marks, and sealed joints.
// PBR lit with point lights. Slightly reflective from polishing.
//
// Binding conventions:
...
struct Material { base_color: vec3<f32>, metallic: f32, roughness: f32, ao: f32, joint_spacing: f32, joint_width: f32, };
...
fn concrete_color(uv: vec2<f32>) -> vec3<f32> {
    let base = material.base_color;
    let an1 = noise(uv * 40.0); let an2 = noise(uv * 80.0);
    let ap = an1 * 0.7 + an2 * 0.3;
    let la = noise(uv * 20.0);
    let light_agg = base * 1.2; let dark_agg = base * 0.7; let sand = base * vec3<f32>(1.07, 1.03, 0.93);
    var color = base;
    if (ap > 0.75) { color = mix(color, light_agg, (ap - 0.75) * 4.0 * 0.4); }
    else if (ap < 0.25) { color = mix(color, dark_agg, (0.25 - ap) * 4.0 * 0.3); }
    if (la > 0.8) { let cv = hash(floor(uv * 20.0)); if (cv > 0.5) { color = mix(color, light_agg, 0.6); } else { color = mix(color, sand, 0.4); } }
    color += sin(uv.x * 200.0 + noise(uv * 10.0) * 2.0) * 0.01;
    let js = 1.0 / material.joint_spacing;
    let jl = fract(uv * js);
    let jf = min(smoothstep(0.0, material.joint_width, jl.x) * smoothstep(0.0, material.joint_width, 1.0 - jl.x), smoothstep(0.0, material.joint_width, jl.y) * smoothstep(0.0, material.joint_width, 1.0 - jl.y));
    return mix(vec3<f32>(0.2, 0.2, 0.22), color, jf);
}
...
fn fs_main(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    let albedo = concrete_color(in.uv);
