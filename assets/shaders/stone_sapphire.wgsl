// stone_sapphire.wgsl — Procedural white stone with sapphire blue veins.
// White stone base with thresholded noise creating blue sapphire vein patterns.
// PBR lit with point lights. Decorative/accent material.
//
// Binding conventions:
...
struct Material {
    stone_color: vec3<f32>, metallic: f32,
    roughness: f32, ao: f32,
    sapphire_color_r: f32, sapphire_color_g: f32,
    sapphire_color_b: f32, vein_scale: f32,
    _pad1: f32, _pad2: f32,
};
...
fn fs_main(in: VertexOutput) -> @location(0) vec4<f32> {
    let sapphire = vec3<f32>(material.sapphire_color_r, material.sapphire_color_g, material.sapphire_color_b);
    let p = in.world_pos.xz * material.vein_scale;
    let n_val = noise2(p);
    let sapphire_mask = smoothstep(0.65, 0.8, n_val) * 0.7 + smoothstep(0.95, 1.0, n_val) * 0.3;
    let albedo = mix(material.stone_color, sapphire, sapphire_mask);
