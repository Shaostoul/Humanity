...
> own section below; the Game Engine table's registered-systems count and the Electrical
> / Solar / Plumbing / Atmosphere rows were corrected to match `tests/engine_wiring_lint.rs`.
> Other sections (Civilization Trust Layer, Marketplace, Wallet & Funding, Web Tools,
> Task Board, etc.) were NOT re-audited this pass and may still lag the binary; treat
> their older rows as "claimed built," not "re-verified 2026-06-30." Prior header
> context (still current as far as known): native voice v0.485-495 (pure-Rust
> capture/Opus/WebRTC, per-channel rooms, live audio web↔native, DSP, transmit modes,
> see the Communication table); website parity v0.469.1-3; inventory/garden UI overhaul
...
| Vehicle/mech system | ✅ | `VehicleSystem` REGISTERED (v0.677, economy Phase 2): kit deploy, factory pad spawn (via CraftingSystem), Summon + self-drive routes all tick; the enter/exit/mech arms stay dormant until Stage 3 take-over wiring. |
...
| Construction system (ECS) | ⚠️ | `ConstructionSystem` + `PlacementSystem` implemented but **NOT registered**, never tick, and this is likely permanent now rather than pending: home building (walls, doors, zones, rail, utilities) shipped v0.463-0.637 on a completely separate, bespoke `HomeStructure` RON model + dedicated in-app editor instead of these two ECS systems. See "Construction/Build Editor" further down for the real, shipping system, and `tests/engine_wiring_lint.rs::DEFERRED_SYSTEMS`. |
...
| Atmospheric system | ✅ | `AtmosphereSystem` **registered + ticking** (v0.617): a sealed home air space, occupancy + a powered air recycler, cut the power and O2 falls (power → air → Vitals). No longer in `tests/engine_wiring_lint.rs::DEFERRED_SYSTEMS` (corrected 2026-06-30, was stale since v0.617). |
| Plumbing system | ✅ | `PlumbingSystem` **registered + ticking** (v0.608): the live home water sim coupled to power, cisterns store, powered pumps/purifiers fill them, fixtures draw, per pipe island; cutting power stops the water. Operates on `WaterTank`/`WaterProducer`/`WaterConsumer`/`PlumbingCircuit` entities. No longer in `DEFERRED_SYSTEMS` (corrected 2026-06-30, was stale since v0.608). |
...
| 15 scaffolded system modules | ⚠️ | `aging`, `astronomy`, `creative_arts`, `docking`, `fire`, `genetics`, `geology`, `governance` (system, not the trust-layer governance), `hvac`, `manufacturing`, `medical`, `oceanography`, `offline`, `transportation`, `waste`, implemented but **NOT registered, so they never tick**. `plumbing` moved OUT of this list 2026-06-30 (registered since v0.608, see above). Deferred with reasons in `tests/engine_wiring_lint.rs::DEFERRED_SYSTEMS`. |
| ⚠️ SYSTEMS-TABLE ACCURACY (updated 2026-06-30, was 2026-05-29) | ⚠️ | Many ✅ rows above mean "implemented in code," NOT "registered + running." As of v0.637.0, `tests/engine_wiring_lint.rs::DEFERRED_SYSTEMS` confirms 17 systems actually tick (as of v0.677: +Vehicle): the original 12 (Time, PlayerController, Interaction, Farming, Inventory, ContainerCompatibility, Crafting, Food, Drone, Weather, Skill, Quest) **plus 4 added since the v0.437-0.628 home-power/utility-wiring arc**: Electrical, Solar, Plumbing (v0.608), Atmosphere (v0.617). The rest (ecology, hydrology, disasters, AI, construction + the 15 remaining scaffolds) compile but are unregistered, they never run. Separately, home construction/building itself (walls, zones, rail, utilities) does NOT run through `ConstructionSystem`/`PlacementSystem` at all, it ships on its own bespoke `HomeStructure` RON model + a dedicated in-app editor (v0.463-0.637), which is why those two ECS systems remain deferred despite construction being one of the most active, most-shipped feature areas in the project. **`tests/engine_wiring_lint.rs` is the authoritative registered-vs-deferred list** (build fails if a system is neither). |
| Electrical + Solar system | ✅ | `ElectricalSystem` + `SolarSystem` **registered + ticking** in `src/lib.rs` (the live home power sim, v0.437-0.606): machines are live ECS power entities (generators/consumers/batteries) that load-shed by priority and charge/discharge with the solar day/night swing. No longer in `DEFERRED_SYSTEMS` (corrected 2026-06-30, this row was stale). |
...
## Construction / Home-Building System (v0.437 to v0.637)

**Added 2026-06-30, this entire arc was previously undocumented here** (STATUS.md's
last full pass predated it; ROADMAP.md's "The simulation" section and FEATURES.md are
the other places this is tracked). This does NOT run through the ECS
`ConstructionSystem`/`PlacementSystem` rows above, those are a separate, still-deferred
approach that building superseded. What actually shipped and works:

| Feature | Status | Details |
|---------|--------|---------|
| In-app construction editor | ✅ | Build the homestead from inside the app: top-down plan, 3D astral camera + room grab, a three-column editor, doors/windows as placed objects (v0.463-471). |
| Construction editor v2 rebuild | ✅ | A fixed outer box + freely-placed interior walls (rooms emerge by flood-fill), data-driven wall materials/thickness/layers, mitred corners, animated doors/windows + locks + control panels, per-home lights, a unified object browser, move/duplicate gizmos, undo/redo, a CAD dimension overlay, a construction console (AI act surface) + live JSON introspection (AI read surface) (v0.532-0.603). |
| Structural pieces | ✅ | Data-driven stairs, ramps, ladders, elevators, teleporters, train rail, decks, and roads as a curved node graph, walkable in first person (v0.583-0.592). |
| Home power sim | ✅ | Runs on the registered `ElectricalSystem`/`SolarSystem` (see Game Engine table above), not a construction-specific system: machines are live ECS power entities, load-shed by priority, charge/discharge with the solar swing (v0.437-0.606). |
| Utility wiring | ✅ | Power, water, air, and data travel through rated cables/pipes with real AWG/ampacity/voltage-drop physics; machines declare IN/OUT ports rendered as real port-node gizmos with viewport drag-to-connect; a live water/plumbing sim runs on the registered `PlumbingSystem` (v0.604-0.628). |
| Mothership superstructure | ⚠️ | Zones (a labelled bounded volume, the macro analogue of a room) and a multi-stop rail node graph with animated cars both shipped (M1, M2/M2b, v0.629-0.637). Paused 2026-06-29 pending an operator decision on the zone-editor architecture, a civic mall design (M3), and multi-home grid tiers (S3); see ROADMAP.md "Right now". |

This is the single most active development area in the project's recent history
(v0.455-0.637) and, until this edit, was the largest gap between this document and the
actual binary.
