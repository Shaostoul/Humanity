...
| ⚠️ SYSTEMS-TABLE ACCURACY (2026-05-29 audit) | ⚠️ | Many ✅ rows above mean "implemented in code," NOT "registered + running." Only 9 systems actually tick: Time, PlayerController, Interaction, Farming, Inventory, ContainerCompatibility, Crafting, Food (nutrition/hunger + spoilage, v0.330.0), Drone (asteroid mining, v0.332.0). The rest (weather, ecology, hydrology, atmosphere, disasters, AI, vehicles, construction, skills, quests + the 16 scaffolds) compile but are unregistered — they never run. **`tests/engine_wiring_lint.rs` is the authoritative registered-vs-deferred list** (build fails if a system is neither). The individual system rows above/below are now downgraded to ⚠️ accordingly (done 2026-05-29). |
