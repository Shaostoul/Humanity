...
### Room identity + the Console room (in-world screens ladder, rung 4)
...
deck") is the household's fixed workstation where the in-world screens mount: zone type
`console_room`, rooms.ron entry `console_room`, and a walled 3.5 x 3 m annex north of the common room
(x 47.5..51.0, z 44.0..47.0) in `ship_structure.ron`, zone `console-room`. Also added room-grade zone
types `room_garden` and `room_living`, and `room_type` on every house zone with a rooms.ron entry.
