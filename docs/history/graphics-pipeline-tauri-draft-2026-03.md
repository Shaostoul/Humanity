# HumanityOS Graphics Pipeline: Design Document (superseded, archived 2026-06-30)

> **Superseded.** Written 2026-03-17, before the Tauri-webview UI was dropped for
> native egui. The wgpu/PBR/deferred-rendering technical content below informed
> the renderer that actually shipped (`src/renderer/`: pipeline, bloom, hologram,
> particles, sky, stars all real and current), but the "dual-surface Tauri
> webview + wgpu window" architecture and the VR/OpenXR roadmap never happened
> and are not planned. Kept for historical rendering-design rationale only. For
> current renderer status see `CLAUDE.md` file map and `docs/STATUS.md`.

# HumanityOS Graphics Pipeline: Design Document

**Status:** Draft (2026-03, historical)
