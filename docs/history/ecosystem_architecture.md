...
> **Archived 2026-06-30. STALE DRAFT, PRE-DATES THE UNIFIED BINARY.** Describes three
> separate surfaces (web / native app / a separate "Immersive Game Surface"), but the real
> architecture (v0.90.0+) is a SINGLE unified binary — the native app IS the game, not a
> third surface. Never finalized past "Draft v0.1"; the `docs/contributor/
> 06-SOURCE-OF-TRUTH-MAP.md` reference to this file even expects a `docs/design/product/`
> subfolder that doesn't exist, confirming broader drift. Moved from `docs/design/` to
> `docs/history/` as a superseded early architecture sketch. See `CLAUDE.md` for the real,
