# Gardening Game: HumanityOS First Minigame (superseded, archived 2026-06-30)

> **Superseded, never built as designed.** This proposed a standalone 2D isometric
> Canvas-2D minigame (`garden-engine.js`/`garden-game.js`/`garden-data.js`, none of
> which exist) with Tauri desktop + a future Bevy 3D port — both dead premises (Tauri
> was dropped for native egui; Bevy was never adopted, HumanityOS uses a custom wgpu
> engine). The real gardening/farming loop went a different direction: a native 3D
> `FarmingSystem` fully wired into the survival gameplay loop (plant/water/harvest,
> nutrition, cooking), see `docs/design/gameplay-loops.md` §4 and §build-order step 4
> (shipped v0.331.0+). The web `web/activities/gardening.html` (1696 lines) is a
> real, separately-evolved page, not built from this spec. Kept for historical record.
