...
> **SUPERSEDED.** This audit predates the v0.90.0 unified-binary restructure; its file
> paths and line numbers (`client/index.html`, `storage.rs`, `server/`) no longer exist.
> At least one finding (H-1, non-constant-time API_SECRET comparison) is already fixed
> in current code (`src/relay/api.rs`, `constant_time_eq`) with no update recorded here.
> For current crypto posture, read CLAUDE.md's Cryptography section. For the live
> security-review cadence and open items, read `docs/SECURITY-CADENCE.md`. Kept here as a
> historical record, not a live tracker of unresolved findings.
