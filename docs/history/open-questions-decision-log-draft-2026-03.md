# Humanity Open Questions & Decision Log (Draft, archived 2026-06-30)

> **Superseded.** An early (2026-03) decision log; every question below is still marked
> "TBD"/"Open" because the log itself was abandoned, not because the questions are
> unresolved, most were settled ad hoc as the product shipped (see `CLAUDE.md`'s
> Cryptography section for identity/E2EE, `docs/ROADMAP.md` for scope decisions). Kept
> for historical record; do not treat any item below as currently open without
> re-checking against current docs first.
