...
> **Archived 2026-06-30. NEVER IMPLEMENTED.** Proposes a pluggable `AiAdapter` interface
> (none/local/cloud/bridge modes, including an `adapter-openclaw`). Zero matches for
> `AiAdapter`, `adapter-none`, or any adapter registry in `src/`. OpenClaw itself (a generic
> personal-assistant tool) was tried twice and rejected twice by the operator. The real AI
> participation model is the game Perception API + chat identity, documented in
> `docs/design/ai_interface.md` and `docs/ai/onboarding.md`. Moved from `docs/design/` to
> `docs/history/` as an unadopted proposal.
