...
> **Archived 2026-06-30.** This "locked" v1 baseline from 2026-03-08 predates the v0.90.0
> unified-binary restructure, the PQ crypto cutover, and most of the current UI/nav. Several
> specific decisions are now factually superseded (e.g. "client-hosted first, servers later" —
> the relay is authoritative from the start per `docs/design/first-playable.md`; the
> "H, Private, Public, Ops, Utility" header order — the real nav is red/green/blue color
> groups, see `web/shared/shell.js`). Its enduring values (no pay-to-win, combat-off-in-v1,
> offline-first, analytics/AI off by default) are already captured in `CLAUDE.md` and
> `docs/ROADMAP.md`/`docs/PRIORITIES.md`. Moved from `docs/design/` to `docs/history/` because
> its "wins until explicitly revised" framing is misleading now that reality diverged without
> a formal revision.

Status: **active baseline** (superseded — see banner above)
Updated: 2026-03-08
