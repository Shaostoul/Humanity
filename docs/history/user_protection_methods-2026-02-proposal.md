...
> **Archived (moved from docs/design/ 2026-06-30).** This external proposal (Signal Double
> Ratchet, MLS groups, X3DH/ECDH key agreement, Kyber+X25519 "hybrid") was never adopted.
> The crypto architecture that actually shipped is pure post-quantum: Dilithium3/ML-DSA-65
> chat identity derived from a BIP39 seed, and DM E2EE via pure Kyber768/ML-KEM-768 ->
> BLAKE3-KDF -> AES-256-GCM dual-seal (no Double Ratchet, no MLS, no classical ECDH — the
> last ECDH path was deleted in the PQ cutover). See the "Cryptography" section of
> `CLAUDE.md` for the current, canonical, shipped design. Kept here only as a historical
> record of an alternate proposal that was considered and not adopted.
