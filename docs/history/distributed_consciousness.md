...
> **Archived 2026-06-30. BUILT ON OPENCLAW, WHICH WAS REJECTED.** This doc describes
> OpenClaw as an adopted, shipped integration ("Phase 1: Complete") and proposes a
> `/api/memory` REST endpoint that was never implemented (no matches in `src/relay/`).
> OpenClaw (a generic personal-assistant tool) was tried twice and rejected twice by the
> operator (added Feb 2026, removed March 2026, resurfaced and removed again 2026-06-30).
> Moved from `docs/design/` to `docs/history/` as a record of an idea that was considered
> and not adopted; the "Heron"/Boisson references are real project lore (see
> `docs/accord/humanity_accord.md`, `docs/website/about.md`) but the architecture here is not.
