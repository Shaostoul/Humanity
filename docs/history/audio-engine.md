...
> **Archived 2026-06-30. PROPOSAL NEVER FULLY ADOPTED.** This doc recommends kira + Steam
> Audio (HRTF, Atmos, 256+ voice pool with bus ducking). The real shipped audio system
> (`src/audio/mod.rs`, `sounds.rs`, `spatial.rs`) uses kira only — no Steam Audio dependency
> was ever added to Cargo.toml, no Atmos, no bus-ducking mixer. Moved from `docs/design/` to
> `docs/history/` as a superseded proposal; see `docs/FEATURES.md` "Audio (kira)" for what's
> actually built.
