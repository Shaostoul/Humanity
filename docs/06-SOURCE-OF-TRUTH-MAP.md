...
4. `crates/`, `desktop/`, `shared/`, `website/`, `game/` (implementation layers)
5. `website/` content is presentation; canonical meaning stays upstream

See also: `website/README.md`.
...
- `desktop/src-tauri` (native wrapper)
- `crates/humanity-relay/client` (web client served by relay)
- `shared/` (shared front-end assets/scripts)
- `website/` (public docs/presentation site)
...
- Implementation now:
  - `crates/humanity-relay/src/{main,relay,api,storage}.rs`
- Status:
  - **Documented: strong**
...
- Design sources:
  ...
  - `design/engine/`
- Implementation now:
  - `game/` has web assets/pages
  - No full Rust game runtime crate integrated in workspace yet
- Status:
...
- Implementation now:
  - `desktop/` (Tauri v2 wrapper around web app)
- Status:
  - **Documented: present**
  - **Implemented: strong**
...
- Implementation now:
  - `website/` + mirrored/public docs strategy
- Status:
  - **Documented: strong**
