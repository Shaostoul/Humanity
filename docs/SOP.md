...
7. **INTERACTABILITY (operator directive 2026-06-15, after this bug recurred on the showroom AND the construction editor):** when you add an in-world egui surface (a `SidePanel`/`Window`/overlay shown while `active_page == None`, e.g. the showroom or construction editor), the panel can RENDER but be un-clickable because the OS cursor is still grabbed for first-person look. BEFORE considering it done, verify ALL of:
   - **No full-screen egui layer is drawn OVER it.** This was the actual recurring cause (v0.461): the HUD draws a full-screen `Area` (`hud.rs`), and a full-screen Area sits in front of an in-world `SidePanel` and eats ALL its clicks even with `Sense::hover()` (it is the topmost layer at the pointer). The HUD is now `.interactable(false)` AND skipped during the showroom + construction editor. So: mark any full-screen HUD/overlay Area `.interactable(false)`, and skip it (like the `!showroom_active && !construction_active` guard) while an in-world panel is up. This is why the showroom "worked" and the editor did not -- the HUD was already skipped for the showroom only.
   - The new active-state is added to **`reconcile_cursor`** in `src/lib.rs` (its `want_free` condition). That function is the SINGLE authority for the cursor grab; do NOT call `set_cursor_grab`/`set_cursor_visible` anywhere else (three stray handlers once did, desyncing `cursor_free` so the reconciliation's transition check silently no-opped).
