...
| Maps | `maps.rs` | Solar system orbit view + planet details, sidebar list grouped by type. | everyone | both |
...
| Studio | `studio.rs` | OBS-like broadcasting studio (scenes, sources, properties). | everyone | web-only (native partial) |
...
| Governance | `governance.rs` | Proposals + votes + tally (local + civilization scope). **Read-only in practice**: there is no create-proposal or cast-vote UI on either client, only display/tally, see `docs/PRIORITIES.md` / the 2026-06-30 gap review. | everyone | both |
