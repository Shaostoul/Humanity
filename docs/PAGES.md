...
| Library | `library.rs` | Everything you READ, in three faces: DOCUMENTS (section > category > document, from `data/library/`, with a tag filter, full-text search, working cross-references, a Contents outline per document and heading anchors, and a rail that folds down to the open document's path plus whatever you unfold by hand, 2026-09-25), the DICTIONARY, and CURRICULUM (v0.1311, `data/curriculum/syllabus.json`), the Library's map of ITSELF: every subject a person needs and honestly how far each has got, with unwritten topics shown dimmed rather than hidden. The external tools/websites third face moved to the Tools page 2026-07-30, so Library and Tools now split by verb rather than by ownership. Web mirror: `library.html`, same manifest. | everyone | both (data-parity enforced) |
...
| Studio | `studio.rs` | OBS-like broadcasting studio (scenes, sources, properties) with a Program/Preview split (v0.664): scene clicks stage into Preview, "Cut to Program" pushes live. Real broadcast transport shipped v0.853-0.855 (GPU window capture, MJPEG frames over a binary WebSocket to the relay at /ws/live/pub). Chat page carries a full-width live strip (v0.1150) with Go Live + the live-now directory. | everyone | native-only, no web page |
| Watch | `watch.rs` | Stream viewer (v0.857): browse live broadcasts on the connected relay (GET /api/live) and watch one (MJPEG frames over /ws/live/sub via `net/live_viewer.rs`; not segments). Sits beside Studio in the nav; the web mirror is `watch.html`. | everyone | both |
| RelayControl | `relay_control.rs` (+ `host_node.rs`) | Server Control Center (v0.846; nav tab renamed "Relays" to "Servers" v0.1130 per federation-ux.md): manage servers you own - health, actions, config; the in-app VPS Console (v0.858) runs server commands over SSH so admin needs no separate terminal. "Host a node on this PC" (`host_node.rs`) starts/stops a relay on THIS machine with a port / database / server-name form, shows the loopback and LAN addresses to hand to a friend, and is the whole page when you have no relay yet; a running local node joins the rail and swaps the VPS-targeted Control + Console sections for its own controls. | admin | native-only |
...
| Planet Tuner | `planet_tuner.rs` | Live per-planet physics editor (v0.1120, a Platform section; artificial-planet ladder item 3): live readout of gravity at the player's altitude, latitude, temperature here vs body reference, catalog pressure + magnetic field, breathability, day length; grouped editors over the frame-locked body's PlanetDef writing changed fields into `data/planets/<id>.ron` via the comment-preserving `assets::ron_edit` (hot reload rebuilds the planet within a frame); gravity_curve raw editor with validated rejects. Dev-mode + cheats gated. | dev | native-only |
| Browser | `browser.rs` | The websites database (`data/web/sites.json`: 10 categories, 38 sites, each with an embed-legality record) as cards. With the readable-web opt-in ON (Settings > Privacy, off by default) a card or a typed address opens INSIDE the app in the readable web view (`widgets/web_view.rs`: html5ever-parsed headings/paragraphs/links/images/lists/tables/code, clickable links, back/forward, no browser engine, no JavaScript, "Open in browser" escape hatch). Off = cards open the OS browser. Design: `docs/design/readable-web.md`. | everyone | both |
...
| Platform | `platform.rs` | The software-itself tab: Recovery, Calculator, Notes, Calendar, Files, Bugs, Testing, Dev, Planet Tuner, Browser. Everything here is the app's OWN machinery. Two things were deliberately pulled out: Settings, per an explicit operator call ("never buried in another menu"), and Tools (2026-07-30), because a catalog of EXTERNAL software and services is not part of the app. Web mirror: `platform.html`. |
...
## Web pages (`web/pages/*.html`: 43 standalone; hi + rules added 2026-09-06; widget-gallery added 2026-08-25; humanity hub added + agents removed 2026-08-17; devlog added 2026-08-01; library + platform added and resources removed 2026-07-30; mission added 2026-07-16; audit/ai-usage/dashboard/data/projects removed 2026-07-05)
...
| Rules | `rules.html` | The published rules for united-humanity.us, who moderates and what powers they actually hold (taken from the enforcing code, not from intentions), how to report, how to appeal including a route that works while banned, and an explicit list of the accountability this server does NOT yet have. Replaces the retired #rules channel whose text is archived in `data/server/legacy_channel_content.json`. Required by `docs/accord/communication_and_association.md`, which says a space must publish its rules and declare its authority before anyone participates. | everyone | yes |
| Hi | `hi.html` | First-contact page at /hi, the QR target on printed business cards. Read once, standing up, by someone who has just met the founder and knows nothing: one plain sentence saying what this is, one thing to do in the next minute (one short growing guide), and the address to come back to. Phone-first and deliberately one screen, so it answers before a thumb reaches to scroll. No jargon by design. | everyone | yes |
...
| Widget Playground | `widgets.html` | Inert widget sandbox + reference (added 2026-08-25): the web home for real widgets AND experimental ones. Demos buttons, a toggle, the shared press-and-hold confirm (modal `holdConfirm` + inline fast-RGB clock ring via `/shared/hold-confirm.js`), the channeling/badge-pulse RGB effects, and an experimental Cistercian-numeral widget (0-9999 as one glyph), with plain-language captions. Nothing triggers a real action; linked from the Platform hub. Native counterpart: the Settings > Widgets "Effects test bench". | everyone (dev/edu) | yes |
...
| Web | `web.html` | Web mirror of the native Browser page (2026-09-16): the same `data/web/sites.json` cards (categories, descriptions, embed review state on hover) opening in a new window, plus the person's own bookmarks in localStorage. The in-app reader itself is native-only (a website cannot host it). | everyone | yes |

| Library | `library.html` | **Web mirror of the native Library** (built 2026-07-30). Document tree + reader + Dictionary + Curriculum, reading the SAME `data/library/index.json` manifest, the SAME markdown files and the SAME `data/curriculum/syllabus.json` native reads from disk. Before this, web exposed only the 17 Accord docs via the relay while the app shipped 53: the single worst parity gap in the registry. Enforced now by `tests/page_parity_lint.rs`. Since 2026-09-25 the page has ONE scrollbar: the rail has no scroller of its own, travels with the page and pins at whichever end you are scrolling toward (`pinRail` in library-app.js). | everyone | both |
