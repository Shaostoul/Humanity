...
1. **Web-mirrors-native parity (Track W — ongoing).** The native chat page is the canonical/parent design; the web chat is the old/broken UI and must be rebuilt as its child (same GUI/UX/pages/style/theme). egui drives the design within its constraints; web mirrors. See `docs/design/studio-streaming.md` §"Track W". Incremental, component-by-component (keeps the working web chat usable throughout), starting with the chat page. Runs in parallel with everything else.
