...
9. **Game/simulator opt-out toggle (NEW, logged 2026-06-30).** Operator: someone who only wants chat, or the Studio (OBS-like) streaming feature, shouldn't pay the compute/GPU/electricity cost of the simulation engine (renderer, ECS tick, terrain, physics) running in the background just because they launched HumanityOS. Today the game/world autoloads on launch unconditionally. Scope: a startup/settings choice (likely `SettingsGraphics` or a new first-run prompt) that skips `load_world`/the renderer's heavy init and the ECS `SystemRunner` tick entirely when the user only wants chat/relay/Studio, while keeping the WebSocket client, chat, identity, and Studio pages fully functional. Cross-reference `docs/ROADMAP.md`'s "The simulation" theme for the roadmap-level entry. Not yet scoped into concrete files, first task is figuring out exactly what `lib.rs`/`main.rs` gates need a "sim disabled" branch without breaking the pages that DO need world state (Homes, Cosmos, Construction, Inventory, Crafting, Quests all assume a loaded world today, decide whether those pages gray out or lazy-load the world on demand).
