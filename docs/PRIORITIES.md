...
**ACTIVE: Web-mirrors-native chat parity rebuild (Track W).** Operator (2026-05-20) chose this as the first build, with native WebRTC transport as the committed parallel next-major-effort. Tracks 0+1 fully closed. The parity rebuild executes `docs/design/web-native-parity.md` incrementally — **migration step 1 = left-rail tabs → stacked collapsible sections** (the biggest divergence; defines the nav feel). Native WebRTC (the weeks-long transport that unblocks native voice + native streaming) is committed but starts as its own focused effort after / alongside, since one agent can't meaningfully advance both at once — its full build plan gets written when it becomes the active focus.
...
1. **Web-mirrors-native parity (Track W — ACTIVE).** Full divergence map + migration order in `docs/design/web-native-parity.md`. Native chat is the parent; web is the old UI being rebuilt to mirror it, incrementally (web stays usable throughout; theme tokens already shared). Migration order: (1) left-rail tabs→stacked-collapsible-sections [NEXT], (2) right-rail align to Friends/Members, (3) message rows + timestamp pill + inline reactions, (4) header + composer, (5) top-nav alignment, (6) spacing sweep + dead-CSS removal. Each step = its own increment + version bump.
