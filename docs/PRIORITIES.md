...
**ACTIVE: Web-mirrors-native chat parity rebuild (Track W).** Operator (2026-05-20) chose this as the first build, with native WebRTC transport as the committed parallel next-major-effort. Tracks 0+1 fully closed. The parity rebuild executes `docs/design/web-native-parity.md` incrementally. **Step 1 (left rail) + Step 2 (right rail) COMPLETE; Step 5 nav labels done.** Shipped: 1 tabs→stacked-sections (pre-existing, verified), 1b studio→right (v0.287.4), 1c scratchpad→top-row (v0.287.8), 1d identity-header→account-menu (v0.287.9); right rail Friends+Members (v0.287.5/.6); nav labels+icons (v0.287.7). **NEXT = step 3: message rows + timestamp pill (Þ) + inline reactions — the biggest CSS surface.** Then step 4 (channel header + composer), step 5-tail (native tiered nav), step 6 (spacing sweep + dead-CSS removal — `web/chat/style.css` and `chat-voice.js` are unloaded/dead). Native WebRTC (the weeks-long transport that unblocks native voice + native streaming) is committed but starts as its own focused effort after / alongside, since one agent can't meaningfully advance both at once — its full build plan gets written when it becomes the active focus.
