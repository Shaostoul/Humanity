...
1. **Machines in the editor (the #1 parity gap).** Add a Machines panel to the construction editor: for the selected room, list its `home.ron` instances; add (from the catalog), remove, nudge offset; render them in the 3D view; Save writes `home.ron` instances. Players can finally place machines; the AI keeps editing the same file. *(First build — chosen 2026-06-23.)*
