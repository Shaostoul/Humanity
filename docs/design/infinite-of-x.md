...
|-------------|------|--------------|---------|
| `src/gui/mod.rs:1048-1058` | 5 planet definitions | `data/star_systems/sol.json` (was `data/solar_system/bodies.json`, moved since) | v0.94.0 |
...
| `src/gui/pages/resources.rs:22-92` | 6 resource categories with 30+ URLs | `data/resources/catalog.json` (RETIRED v0.415.0; the external catalog is now `data/external/catalog.json`) | v0.123.0 |
...
| Where | What | Target file | Notes |
|-------|------|-------------|-------|
| `src/gui/pages/ai_usage.rs:93-94` | AI provider list + time window choices | `data/ai_usage/filters.json` | RETIRED with the ai-usage page (v0.197.0); kept here as a record of the migration, not as a live path. |
...
- **External catalog**: `data/external/catalog.json` → the Tools page on both clients (replaced `data/tools/` + `data/resources/` in 2026-07).
