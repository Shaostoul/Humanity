...
| ~~`src/gui/mod.rs:1048-1058`~~ | **Migrated in v0.94.0.** Planets now load from `data/solar_system/bodies.json` via `load_planets()`. Our solar system is the first data set the simulation ships with; additional systems are added as new files, no code changes. | Done. |
