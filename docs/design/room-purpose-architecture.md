...
> **The long-term construction architecture is designed in
> [`construction-architecture.md`](construction-architecture.md)** (v0.456): a five-layer
> pipeline (rooms as a derived semantic view; an editable wall-graph boundary that generalizes
> from today's bounding boxes to non-grid / organic shapes; a generator that turns boundaries
> into real framing - studs/joists/sheathing; a node-beam structural solver for load-bearing +
> destruction; the render layer). It explains how "room placement first" is slice 1 of the same
> system that later supports organic alien hulls, real physics loads, and blow-a-hole
> destruction with decompression, plus a staged build plan and what to prototype first.
