...
- **Phase 2 — Signed + E2EE messages.** `group_msg_v1` signed by author + epoch-key AES-GCM. Epoch re-key on membership change (seal to each Kyber pub). *Exit:* relay never sees group plaintext; removed members can't read new epochs.
