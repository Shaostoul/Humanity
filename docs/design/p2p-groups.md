...
- **Phase 3 — P2P transport.** Full-mesh DataChannels for live group messaging; relay drops to signaling-only. *Exit:* messages flow with the relay carrying zero message bytes.
  - **⚠️ Signaling bug — the v0.309.2 mesh was NON-FUNCTIONAL until v0.314.0.** `chat-p2p.js` sent the DataChannel offer/answer/ICE as `{type:'webrtc_signal', target:…}` but the relay's `RelayMessage::WebrtcSignal` required `from`+`to` (no serde default/alias) → serde dropped every signaling frame → no DataChannel ever opened → group/DM P2P silently fell back to the relay poll the whole time. (Voice was unaffected — it sends from+to.) **Fixed v0.314.0:** relay `from` → `#[serde(default)]` (it's overwritten with the authenticated sender key anyway), `to` → `#[serde(alias="target")]` (deployed web clients route with no refresh); web aligned to canonical `to`. Re-verify web↔web instant delivery before trusting the mesh. This is also the prerequisite for native P2P push (native↔web interop over the same signaling).
  - **Increment 1 — SHIPPED v0.309.2 (web), ACTIVATED v0.314.0.** Live group messages broadcast over WebRTC DataChannels to connected roster members, ADDITIVE to the relay POST (relay = cache + offline backfill, not removed). Reuses `chat-p2p.js` transport + `webrtc_signal` (no new relay handler; relay signaling-only). New `pq-object.js verifyObjectSubmission` lets a peer verify a pushed `group_msg_v1` locally (same ML-DSA check the relay runs) + dedup by `object_id` against the poll. Glare-free mesh (larger-pubkey offers). `scripts/object-verify-kat.mjs` locks the verify primitive. **Needs operator two-identity browser test before increment 2.**
