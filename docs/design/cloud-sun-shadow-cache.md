...
OPEN QUESTIONS:
Q1 (RESOLVED v0.1291: 0.0, the ladder, after finding F1; 1.0 is the pre-fix twin only, its band printed black over built lobes at the coarse edge): shipped default for the far fallback - the ladder (current, CLOUD_LC_FAR_ANALYTIC = 0.0) or the analytic column (1.0)? The critic's horizon-vantage note applies: measure one horizon look before declaring the perf gate met, or set 1.0 for that measurement.
