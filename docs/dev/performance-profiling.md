...
| `debug/screen_request.json` | Drives an in-world wall screen's page (hover / click / scroll / text / find / link / snapshot / wait_ready) through the same event API the look ray uses; see `docs/design/in-world-screens.md`, "Dev IPC" | `debug/screen_done.json` |
| `debug/ui_request.json` (2026-09-18) | Drives the MAIN egui context: `{"action":"f10"}` / `"escape"` run the F10 sidebar's key rules (the same functions the keys call), `{"action":"click","pos":[x,y]}` / `"pointer_move"` push synthetic pointer events (window pixels; a click is move, press, release on three frames), `{"action":"find","text":"Depth dither"}` locates a drawn text on the main frame, `"state"` just reports. Optional `"flag":"<GuiState cloud_dev_* field>"` is reported before and after | `debug/ui_request_done.json`: `ok`, `cursor_free` (winit), `gui_cursor_free`, `cursor_want_free` (read this one in a background rig, where the window is never touched), `background_no_cursor`, `cloud_dev_sidebar_expanded`, `active_page`, egui's `wants_pointer_input` + `is_pointer_over_area`, `flag_before` / `flag_after`, `closed` (escape), `found` / `matches` / `rect_px` / `pos_px` (find; `pos_px` is the next `click`'s `pos`) |
